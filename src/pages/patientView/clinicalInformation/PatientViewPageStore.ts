...
export class PatientViewPageStore {
    ...
    setGeneticCounselingRecommended = (value: boolean) => {
        ...
        request
            .put(
                this.getJsonStoreUrl() +
                    this.getSafePatientId() +
                    '/geneticCounselingRecommended'
            )
            .set('Content-Type', 'application/json')
            .send(JSON.stringify({ geneticCounselingRecommended: value }))
            .end((err, res) => {
                if (!err && res.status === 200) {
                    console.log(
                        'Success editing genetic counseling ' +
                            this.getSafePatientId()
                    );
                } else {
                    console.log(
                        'Error editing genetic counseling ' +
                            this.getSafePatientId()
                    );
                }
            });
    ...
    setRebiopsyRecommended = (value: boolean) => {
        ...
        request
            ...
            .set('Content-Type', 'application/json')
            .send(JSON.stringify({ rebiopsyRecommended: value }))
            .end((err, res) => {
                if (!err && res.status === 200) {
                    console.log(
                        'Success editing rebiopsy recommendation ' +
                            this.getSafePatientId()
                    );
                } else {
                    console.log(
                        'Error editing rebiopsy recommendation ' +
                            this.getSafePatientId()
                    );
                }
            });
    ...
    setCommentRecommendation = (value: string) => {
        ...
        request
            .put(this.getJsonStoreUrl() + this.getSafePatientId() + '/comment')
            .set('Content-Type', 'application/json')
            .send(JSON.stringify({ comment: value }))
            .end((err, res) => {
                if (!err && res.status === 200) {
                    console.log(
                        'Success editing comment ' + this.getSafePatientId()
                    );
                } else {
                    console.log(
                        'Error editing comment ' + this.getSafePatientId()
                    );
                }
            });
    };
    ...
    get therapyRecommendations(): ITherapyRecommendation[] {
        ...
        if (
            !this._therapyRecommendations ||
            this._therapyRecommendations.length == 0
        )
            this.loadTherapyRecommendations();
    ...
    therapyRecommendationOnDelete = (
        ...
        let therapyRecommendationToDeleteFlattened = flattenObject(
            therapyRecommendationToDelete
        );
        this._therapyRecommendations = this._therapyRecommendations.filter(
            (therapyRecommendation: ITherapyRecommendation) =>
                therapyRecommendationToDeleteFlattened.id !==
                therapyRecommendation.id
        );
        this.deleteTherapyRecommendation(
            therapyRecommendationToDeleteFlattened.id
        );
        return true;
    };
    ...
    therapyRecommendationOnAddOrEdit = (
        ...
        let therapyRecommendationToAddFlattened = flattenObject(
            therapyRecommendationToAdd
        );
        this._therapyRecommendations = this._therapyRecommendations.filter(
            (therapyRecommendation: ITherapyRecommendation) =>
                therapyRecommendationToAddFlattened.id !==
                therapyRecommendation.id
        );
    ...
    private loadTherapyRecommendations() {
        request
            .get(this.getJsonStoreUrl() + this.getSafePatientId())
            .end((err, res) => {
                if (!err && res.status === 200) {
                    ...
                    this.geneticCounselingRecommended =
                        patient.geneticCounselingRecommended || false;
                    this.rebiopsyRecommended =
                        patient.rebiopsyRecommended || false;
    ...
    private addTherapyRecommendation(
        therapyRecommendation: ITherapyRecommendation
    ) {
        request
            .post(
            ...
            .set('Content-Type', 'application/json')
            .send(JSON.stringify(therapyRecommendation))
            .end((err, res) => {
                if (!err && res.status === 201) {
                    console.log(
                        'Success adding therapyRecommendation ' +
                            therapyRecommendation.id
                    );
                } else {
                    console.log(
                        'Error adding therapy recommendation ' +
                            therapyRecommendation.id
                    );
                }
            });
    ...
    private editTherapyRecommendation(
        therapyRecommendation: ITherapyRecommendation
    ) {
        request
            .put(
            ...
            .end((err, res) => {
                if (!err && res.status === 200) {
                    console.log(
                        'Success editing therapy recommendation ' +
                            therapyRecommendation.id
                    );
                } else {
                    console.log(
                        'Error editing therapy recommendation ' +
                            therapyRecommendation.id
                    );
                }
            });
    ...
    private deleteTherapyRecommendation(deleteId: string) {
        request
            ...
            .end((err, res) => {
                if (!err && res.status === 200) {
                    console.log(
                        'Success deleting therapy recommendation ' + deleteId
                    );
                } else {
                    console.log(
                        'Error deleting therapy recommendation ' + deleteId
                    );
                }
            });
    }
